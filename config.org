...
;; enable auto-fill-mode for all text modes
...
When I use auto-fill-mode, I tend to want it to behave the same no matter the
major-mode, so define that behavior here and then just add it to mode hooks
elsewhere as needed.
#+BEGIN_SRC emacs-lisp
...
** DONE Enable pylint to check Python buffers
...
This is likely just using emacsclient commands in $HOME/.mutt/muttrc
** DONE Code folding in Python files
** DONE Configure ERC
This seems to be working - not a huge fan of the colors though
** TODO Add encryption to ERC passwords
** TODO Org-mode should always open org files fully collapsed
** TODO Tell org-mode where to find agenda files
